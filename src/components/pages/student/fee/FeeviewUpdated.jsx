...
const FeeView = () => {
    ...
    const handleAdmissionFee = async () => {
        if (!validateAdmissionFeeInputs()) {
            return
        }
        const formData = {
            amount: admissionFee.initialamount,
            name: "irshad",
            contact: "8186831441",
        ...
        if (admissionFee.initialamount > 0 && admissionFee.paiddate && admissionFee.modeofpayment === "Online Payment") {
            try {
                const response = await createOrder(formData);
                const res = await response.json();

                if (res.success) {
                    handleRazorpayAdmissionPayment(res)
        ...
        if (admissionFee.initialamount > 0 && admissionFee.paiddate && admissionFee.modeofpayment === "Cash") {
    ...
    const validateAdmissionFeeInputs = () => {
        if (isNaN(admissionFee.initialamount)) {
            admissionFee.initialamount = 0
        }

        if (admissionFee.initialamount == null) {
            admissionFee.initialamount = 0
        }
        ...
        if (!admissionFee.paiddate) {
            alert('Enter paid date');
            return false;
        }

        if (!admissionFee.modeofpayment) {
    ...
    const handleRazorpayAdmissionPayment = (res) => {
        const options = {
            key: res.key_id,
            amount: res.amount,
            currency: 'INR',
