...
function Addassetsview() {
  ...
  const getdata = async () => {
    const res = await fetch(
      `${process.env.REACT_APP_API_URL}/addassetsview/${id}`,
      {
        method: "GET",
        headers: {
  ...
  const deleteuser = async (id) => {
    const res2 = await fetch(
      `${process.env.REACT_APP_API_URL}/addassetsdelete/${id}`,
      {
        method: "DELETE",
        headers: {
          "Content-Type": "application/json",
  ...
  return (
    <div className="container mt-3">
      <div className="assignassets-view">
        <h5 className="text-center my-3"> Assets View</h5>
        <div className="d-flex justify-content-evenly">
          <div>
            <h5 className="mt-3">
              <b> Name: </b>
              <span>{getassigndata.vendername}</span>
            </h5>
